...
def evaluate_mse(a_pde, is_term=False):
    ...
    for ix, term in enumerate(terms):
        ...
        if not any(tree_list[0][0].cache.reshape(-1)):  # 如果全是0，无法收敛且无意义
            ...
            # 强制 flatten cache，保证 shape 一致
            cache_val = tree_list[0][0].cache
            if cache_val.ndim == 1:
                cache_val = cache_val.reshape(-1, 1)
            elif cache_val.ndim == 2 and cache_val.shape[1] != 1:
                cache_val = cache_val.reshape(-1, 1)
            terms_values[:, ix:ix+1] = cache_val
            tree_list[0][0].cache = tree_list[0][0].var  # 重置缓冲池
            # print('not 0')
            # pdb.set_trace()

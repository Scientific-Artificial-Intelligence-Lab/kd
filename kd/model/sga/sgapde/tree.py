...
class Tree: #对应于pde中的一个term
    def __init__(self, context, max_depth, p_var):
        self.max_depth = max_depth
        self.tree = [[] for i in range(max_depth)]
        self.preorder, self.inorder = None, None

        root = context.ROOT[np.random.randint(0, len(context.ROOT))] # 随机产生初始root（一种OPS）# e.g. ['sin', 1, np.sin], ['*', 2, np.multiply] 
            ...
            for parent_idx in range(len(self.tree[depth - 1])): #一个tree中某个depth处的node的子节点可以是多个，因此有可能在某个深度处存在多个node
                ...
                for j in range(parent.child_num):
                    ...
                    if parent.name in {'d', 'd^2'} and j == 1: # j == 0 为d的左侧节点，j == 1为d的右侧节点
                        node = context.den[np.random.randint(0, len(context.den))] # 随机产生一个微分运算的denominator，一般是xyt
                    ...
                    # rule 2: 最底一层必须是var，不能是op
                        ...
                        node = context.VARS[np.random.randint(0, len(context.VARS))]
                        node = Node(depth=depth, idx=len(self.tree[depth]), parent_idx=parent_idx, name=node[0],
                                    var=node[2], full=node, child_num=int(node[1]), child_st=None)
                        self.tree[depth].append(node)
                    else:
                    # rule 3: 不是最底一层，p_var概率产生var，如果产生var，则child_st为None。当产生的是ops的时候，产生对应node时对应的child_st会通过计算获得。以便对应于下一层中该ops对应的子节点。
                        if np.random.random() <= p_var:
                            ...
                            node = context.OPS[np.random.randint(0, len(context.OPS))]
                            node = Node(depth=depth, idx=len(self.tree[depth]), parent_idx=parent_idx, name=node[0],
    ...
    def mutate(self, context, p_mute): #直接替换原有tree中的某个节点，用同类型节点替换，因此后续位置不需要重新生成（类似替换了一个基因，而不是把后续基因序列重新产生，具有物理含义，也易于实现）
        global see_tree
        ...
        while depth < self.max_depth:
            ...
            for parent_idx in range(len(self.tree[depth - 1])):
                ...
                for j in range(parent.child_num):  # parent 的第j个子节点
                    ...
                    # print('mutate!')
                    if num_child == 0: # 叶子节点
                        node = context.VARS[np.random.randint(0, len(context.VARS))] # rule 2: 叶节点必须是var，不能是op
                        while node[0] == temp or (parent.name in {'d', 'd^2'} and node[0] not in context.den[:, 0]):# rule 3: 如果编译前后结果重复，或者d的节点不在den中（即出现不能求导的对象），则重新抽取
                            if context.simple_mode and parent.name in {'d', 'd^2'} and node[0] == 'x': # simple_mode中，遇到对于x的导数，直接停止变异
                                break
                            node = context.VARS[np.random.randint(0, len(context.VARS))] # 重新抽取一个vars
                        ...
                        if num_child == 1:
                            node = context.OP1[np.random.randint(0, len(context.OP1))]
                            while node[0] == temp:  # 避免重复
                                node = context.OP1[np.random.randint(0, len(context.OP1))]
                        elif num_child == 2:
                            node = context.OP2[np.random.randint(0, len(context.OP2))]
                            right = self.tree[depth + 1][current.child_st + 1].name
                            while node[0] == temp or (node[0] in {'d', 'd^2'} and right not in context.den[:, 0]):# rule 4: 避免重复，避免生成d以打乱树结构（新d的右子节点不是x）
                                node = context.OP2[np.random.randint(0, len(context.OP2))]
                        else:

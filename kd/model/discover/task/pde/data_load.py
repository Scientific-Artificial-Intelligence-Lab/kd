...
from .utils_fd import *
from .parameter_process import parametric_burgers_rhs
from .utils_subgrid import *
from .utils_noise import cut_bound


def load_data(dataset, training=False):
    """
    ...
    if dataset == 'chafee-infante': # 301*200
        ...
        u = np.load("./discover/task/pde/data_new/chafee_infante_CI.npy")
        x = np.load("./discover/task/pde/data_new/chafee_infante_x.npy").reshape(-1,1)
        t = np.load("./discover/task/pde/data_new/chafee_infante_t.npy").reshape(-1,1)
        n_input_var = 1
        ...
        data = scio.loadmat('./discover/task/pde/data_new/burgers.mat')
        u=data.get("usol")
        ...
        u=np.load("./discover/task/pde/data_new/PDE_divide.npy").T
        nx = 100
        nt = 251
        ...
        u=np.load("./discover/task/pde/data_new/PDE_compound.npy").T
        nx = 100
        nt = 251
        x=np.linspace(1,2,nx).reshape(-1,1)
        ...
        data = scipy.io.loadmat('./discover/task/pde/data_new/burgers2.mat')
        ...
        data=scipy.io.loadmat('./discover/task/pde/data/fisher_nonlin_groundtruth.mat')
        ...
        data=scipy.io.loadmat('./discover/task/pde/data/fisher_groundtruth.mat')
        ...
        data_path = './discover/task/pde/data/loam_S1'
        ...
        data = scipy.io.loadmat('./discover/task/pde/data/matlab_ke_example.mat')
...
def load_data_2D(dataset, training=False):
    # from discover.task.pde.utils_v2 import *
    X= []
    data_dir = f'./discover/task/pde/noise_data'
    n_state_var = 1
    if dataset == 'Cahn_Hilliard_2D':
        path='./discover/task/pde/data/ch.npz'
        path='./discover/task/pde/data/ch_ac.npz'
        path='./discover/task/pde/data_new/noise_ch.npz'
        ...
        path = './discover/task/pde/data_new/bcpinn_ac.npz'
...
def load_data_MD_NU(dataset, noise_level=0, data_amount = 1, training = False, cut_ratio =0.03):
    if "2D" in dataset:
        ...
        if dataset == 'ns_MD_NU':
            data = scipy.io.loadmat('./discover/task/pde/data/Vorticity_ALL.mat')        
            steps = 151
            n = 449
            m = 199
            dt = 0.2
            ...
            center_box = scipy.io.loadmat('./discover/task/pde/data/domain_profile.mat')
            center_box_coordinates  = scipy.io.loadmat('./discover/task/pde/data/domain_coordinates.mat') #127*127*500
        ...
        data = np.load('./discover/task/pde/data/RD_3D.npz')

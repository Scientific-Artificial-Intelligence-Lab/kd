...
class Node(object):
    ...
    def to_sympy_string(self):
        ...
        # --- 根据 KD_Discover 的操作名 (node_op_name) 映射到 SymPy 的函数调用字符串格式 ---
        ...
        elif base_op_name == "diff": # KD_Discover 的 diff(f, x) 对应 SymPy 的 Derivative(f, x)
            return f"Derivative({child_sympy_strings[0]}, {child_sympy_strings[1]})"
        elif base_op_name == "diff2": # KD_Discover 的 diff2(f, x) 对应 SymPy 的 Derivative(f, x, x)
            return f"Derivative({child_sympy_strings[0]}, {child_sympy_strings[1]}, {child_sympy_strings[1]})"
        elif base_op_name == "diff3": # KD_Discover 的 diff3(f, x) 对应 SymPy 的 Derivative(f, x, x, x)
            return f"Derivative({child_sympy_strings[0]}, {child_sympy_strings[1]}, {child_sympy_strings[1]}, {child_sympy_strings[1]})"
        elif base_op_name == "diff4": # KD_Discover 的 diff4(f, x) 对应 SymPy 的 Derivative(f, x, x, x, x)

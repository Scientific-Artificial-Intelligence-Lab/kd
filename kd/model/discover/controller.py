"""Pytorch version of dso controller used to generate distribution over hierarchical, variable-length objects."""
import torch
...
class Controller(object):
    ...
    def loop_fn(self, time, cell_output, cell_state, loop_state):
        ...
        if cell_output is None: # time == 0
            ...
            next_lengths = torch.where(
                    ...
                    lengths,
                    torch.tensor((time+1),dtype = torch.float32).unsqueeze(0).repeat(self.batch_size))
    ...
    def sample_episodes(self, batch_size):
        ...
        time = 0
        finished, next_input, initial_state, emit_output, loop_state = self.loop_fn(time, \
            cell_output=None, cell_state=None, loop_state=None)
        state = initial_state
        while not all(finished):
            
            (output, cell_state) = self.cell(next_input, state)
            time = time + 1
            (finished, next_input, next_state, emit, loop_state) = self.loop_fn(time, \
                cell_output=output, cell_state=cell_state, loop_state=loop_state)

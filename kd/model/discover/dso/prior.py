...
from ..library import TokenNotFoundError
...
class RelationalConstraint(Constraint):
    ...
    def __call__(self, actions, parent, sibling, dangling):
        ...
        if self.relationship == "descendant":
            ...
            unary_effectors = np.intersect1d(self.effectors, #返回两者中的共同元素
...
class NoInputsConstraint(Constraint):
    ...
    def validate(self):
        if len(self.library.non_state_tokens) == 0:
            message = "All terminal tokens are input variables, so all" \
                "sequences will have an state variable."
            return message
        return None
    ...
    def __call__(self, actions, parent, sibling, dangling):
        ...
        # 2) there are no state variables
        mask = (dangling == 1) & \
               (np.sum(np.isin(actions, self.library.state_tokens), axis=1) == 0)
        # import pdb;pdb.set_trace()
        prior = self.make_constraint(mask, self.library.non_state_tokens)
        return prior

    def is_violated(self, actions, parent, sibling):
        # Violated if no input tokens are found in actions
        tokens = self.library.state_tokens
        return bool(np.isin(tokens, actions).sum() == 0)

    def describe(self):
        message = "{}: Sequences contain at least one state variable Token.".format(self.__class__.__name__)
        return message
...
class SubgridDesConstraint(RelationalConstraint):
    def __init__(self,library, targets=None, effectors = None):
        targets = ['add']
        effectors = ["laplacian", "adv","ddx", "ddy"]
        super(SubgridDesConstraint, self).__init__(library=library,
                                        targets=targets,
                                        effectors=effectors,
                                        relationship="descendant")   

class SubgridDesConstraint2(RelationalConstraint):
    def __init__(self,library, targets=None, effectors = None):
        effectors = ['adv']
        targets = ["laplacian"]
        super(SubgridDesConstraint2, self).__init__(library=library,
                                        targets=targets,
                                        effectors=effectors,
                                        relationship="descendant")   
...
class DiffConstraint_left(RelationalConstraint):
    ...
    def __init__(self, library):
        ...
        targets.extend(library.input_tokens.tolist())
        # targets.append(library.const_token)
        effectors = []
...
class DiffConstraint_left_strict(RelationalConstraint):
    ...
    def __init__(self, library):
        """only u and diff could be the left childs"""
...
class DiffConstraint_right(RelationalConstraint):
    ...
    def __call__(self, actions, parent, sibling, dangling):
        assert self.relationship == "rchild", f"relationship is not compatiable"
        parents = self.effectors
...
class DiffConstraint_des(RelationalConstraint):
    def __init__(self, library, extra = None):
        not_included_names  = []
...
class LaplaceConstraint(RelationalConstraint):
    ...
    def __init__(self, library, extras):
        ...
        targets = [token.name for token in library.tokens if token.state_var is None]
        effectors = [name for name in library.names if 'lap' in name or "lap_t" in name]
        for extra in extras:

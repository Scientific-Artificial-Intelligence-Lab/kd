...
def calculate_terms(word,Net,database,variables,nx,nt):
    if 'x' in variables:
    ...
    if word=='sqrt(u)':
        return (torch.sqrt(torch.abs(u))).cpu().data.numpy()
    if word=='sin(u)':
    ...
    if word=='env(u)':
        u = Net(database).cpu().data.numpy().reshape(nt, nx)
        Hilbert_u = np.zeros_like(u, dtype=np.complex64)
        Hilbert_u[:, 0:100] = hilbert(u[:, 0:100], axis=1)
        Hilbert_u[:, 100:200] = hilbert(u[:, 100:200], axis=1)
        Hilbert_u[:, 200:300] = hilbert(u[:, 200:300], axis=1)
        envlope = np.abs(Hilbert_u).reshape(-1,1)
        return envlope

...
def read_dataset(Equation_name=''):
    df=pd.read_excel(_DIR / "PDE_dataset_0725.xlsx")
...
def get_words(dataset):
    ...
    json.dump(dict_datas, open(_DIR / 'dict_datas_0725.json', 'w', encoding='utf-8'))
...
if __name__=='__main__':
    dataset=read_dataset()
    print(len(dataset))
    #calculate_words(dataset)
    get_words(dataset)
    # dataset=get_tran_dataset_different_num('',2)
    # print(len(dataset))
    # formula = '(u^4)xx,+,u^2,/,ux,*,ux,+,(x+y),*,uy'.split(',')
    # sentence = [word2id[word] for word in formula]
    # sentence.append(1)
    # data_augumentation_multiply(sentence)

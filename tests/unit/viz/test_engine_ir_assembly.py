...
from kd.viz import build_integration_result
...
def _capture_assembled_rhs(
    ...
    def _stub(rhs: object, ds: PDEDataset, **kwargs: object) -> IntegrationResult:
        captured.append(rhs)
        assert ds is dataset, "assembly must forward the dataset unchanged"
    ...
    with patch("kd.core.integrator.integrate_pde", side_effect=_stub):
        _integration, notes = build_integration_result(result, dataset)
...
class TestIrRhsAssembly:
    ...
    def test_rhs_assembled_from_terms_and_coeffs_via_repr(
        ...
        rhs, notes = _capture_assembled_rhs(result, dataset)
    ...
    def test_protected_semantics_note_for_log_rhs(
        ...
        assert "protected" in notes[0].lower()
    ...
    def test_protected_semantics_note_for_nested_exp_rhs(
        ...
        _rhs, notes = _capture_assembled_rhs(result, dataset)

        assert len(notes) == 1
        assert "exp" in notes[0]
        assert "protected" in notes[0].lower()

    def test_protected_semantics_note_for_n2_rhs_names_the_operator(
        self, mock_experiment_result: ExperimentResult
    ) -> None:
        dataset = _make_smooth_dataset()
        result = _with_terms(mock_experiment_result, ["n2(u)"], [0.5], None)

        _rhs, notes = _capture_assembled_rhs(result, dataset)

        assert len(notes) == 1
        assert "n2" in notes[0]
        assert "protected" in notes[0].lower()
    ...
    def test_non_finite_coefficient_fails_loud_with_accurate_message(
        ...
        with pytest.raises(ValueError, match="non-finite coefficient") as excinfo:
            build_integration_result(result, dataset)
    ...
    def test_all_pruned_yields_zero_rhs_and_constant_field(
        ...
        with patch("kd.core.integrator.integrate_pde", side_effect=_spy):
            integration, notes = build_integration_result(result, dataset)

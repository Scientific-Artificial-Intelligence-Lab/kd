...
_DATA_PATH: Path = REFERENCE_DATA_DIR / "fisher_nonlin_groundtruth.mat"


_MIN_DATA_BYTES: int = 50_000
_SKIP_REASON = (
    f"Fisher_nonlinear paper data not found (or under "
...
FISHER_NONLINEAR_OPERATORS: list[str] = [
...
def fisher_nonlinear_components() -> _PlatformBundle:
    from kd.search.discover.data.loader import load_fisher_nonlinear_mat

    dataset = load_fisher_nonlinear_mat(_DATA_PATH)
...
def sanity_config() -> DiscoverConfig:
    ...
    return DiscoverConfig(
        ...
        library=LibraryConfig(
            operators=FISHER_NONLINEAR_OPERATORS,
...
def test_engine_completes_without_raising(
    fisher_nonlinear_components: _PlatformBundle,
    ...
    engine.run(
        fisher_nonlinear_components.evaluator,
        n_iterations=SANITY_ITERATIONS,
    )
...
def test_best_reward_above_sanity_threshold(
    ...
    assert state.best_reward > SANITY_MIN_REWARD, (
        ...
        f"Fisher_nonlinear at 20 iter or the reward calc is broken."
    )
...
def test_controller_explores_ground_truth_tokens(
    ...
    assert any(
        ...
        f"Fisher_nonlinear GT-relevant call from {GT_TOKEN_CALLS}; "
        f"sample: {sorted(expressions)[:5]}"
    )
...
def test_ground_truth_expression_is_reachable(
    ...
    assert validator.validate_single(token_ids), (
        f"CandidateValidator rejected Fisher_nonlinear GT preorder "

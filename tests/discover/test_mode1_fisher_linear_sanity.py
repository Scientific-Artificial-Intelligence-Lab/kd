...
_DATA_PATH: Path = REFERENCE_DATA_DIR / "fisher_groundtruth.mat"


_MIN_DATA_BYTES: int = 50_000
_SKIP_REASON = (
    f"Fisher_linear paper data not found (or under {_MIN_DATA_BYTES} "
...
FISHER_LINEAR_OPERATORS: list[str] = [
...
GROUND_TRUTH_TOKENS: list[str] = [
    "add", "diff2_x", "u", "add", "u", "n2", "u",
]


GT_TOKEN_CALLS: tuple[str, ...] = ("diff2_x(", "n2(")
...
def fisher_linear_components() -> _PlatformBundle:
    from kd.search.discover.data.loader import load_fisher_linear_mat

    dataset = load_fisher_linear_mat(_DATA_PATH)
...
def sanity_config() -> DiscoverConfig:
    ...
    return DiscoverConfig(
        ...
        library=LibraryConfig(
            operators=FISHER_LINEAR_OPERATORS,
...
def test_engine_completes_without_raising(
    fisher_linear_components: _PlatformBundle,
    ...
    engine.run(
        fisher_linear_components.evaluator,
        n_iterations=SANITY_ITERATIONS,
    )
...
def test_best_reward_above_sanity_threshold(
    ...
    assert state.best_reward > SANITY_MIN_REWARD, (
        ...
        f"Fisher_linear at 20 iter or the reward calc is broken."
    )
...
def test_controller_explores_ground_truth_tokens(
    ...
    assert any(
        ...
        f"Fisher_linear GT-relevant call from {GT_TOKEN_CALLS}; sample: "
...
def test_ground_truth_expression_is_reachable(
    ...
    assert validator.validate_single(token_ids), (
        f"CandidateValidator rejected Fisher_linear GT preorder "

...
_SKIP_REASON = (
    f"PDE_compound paper data not found (or under {_MIN_DATA_BYTES} "
...
PDE_COMPOUND_OPERATORS: list[str] = [
...
GT_TOKEN_CALLS: tuple[str, ...] = ("mul(", "diff2_x(")
...
def pde_compound_components() -> _PlatformBundle:
    from kd.search.discover.data.loader import load_pde_compound_npy

    dataset = load_pde_compound_npy(_DATA_PATH)
...
def sanity_config() -> DiscoverConfig:
    ...
    return DiscoverConfig(
        ...
        library=LibraryConfig(
            operators=PDE_COMPOUND_OPERATORS,
...
def test_engine_completes_without_raising(
    ...
    engine.run(
        pde_compound_components.evaluator,
        n_iterations=SANITY_ITERATIONS,
    )
...
def test_best_reward_above_sanity_threshold(
    ...
    assert state.best_reward > SANITY_MIN_REWARD, (
        ...
        f"PDE_compound at 20 iter or the reward calc is broken."
    )
...
def test_controller_explores_ground_truth_tokens(
    ...
    assert any(
        ...
        f"PDE_compound GT-relevant call from {GT_TOKEN_CALLS}; "
...
def test_ground_truth_expression_is_reachable(
    ...
    assert validator.validate_single(token_ids), (
        f"CandidateValidator rejected PDE_compound GT preorder "

...
_DATA_PATH: Path = REFERENCE_DATA_DIR / "Kdv.mat"


_MIN_DATA_BYTES: int = 50_000
_SKIP_REASON = (
    f"KdV paper data not found (or under {_MIN_DATA_BYTES} bytes) "
    f"at {_DATA_PATH}."
)
...
GT_TOKEN_CALLS: tuple[str, ...] = ("mul(", "diff3_x(")



KDV_FD_MAX_ORDER: int = 3
...
def kdv_components() -> _PlatformBundle:
    from kd.search.discover.data.loader import load_kdv_mat

    dataset = load_kdv_mat(_DATA_PATH)
    provider = FiniteDiffProvider(dataset, max_order=KDV_FD_MAX_ORDER)
...
def test_engine_completes_without_raising(
    kdv_components: _PlatformBundle,
...
def test_best_reward_above_sanity_threshold(
    ...
    assert state.best_reward > SANITY_MIN_REWARD, (
        ...
        f"KdV at 20 iter or the reward calc is broken."
    )
...
def test_controller_explores_ground_truth_tokens(
    ...
    assert any(
        ...
        f"KdV GT-relevant call from {GT_TOKEN_CALLS}; sample: "
...
def test_ground_truth_expression_is_reachable(
    ...
    assert validator.validate_single(token_ids), (
        f"CandidateValidator rejected KdV GT preorder "

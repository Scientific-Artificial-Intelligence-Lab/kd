...
- [ ] Phase 4: 镜像制作（Bohrium 内置工具或 Dockerfile）、上架发布
  ...
  - [x] ~~持久化存储 `/tmp/` → `/personal/.kd/`~~ → 最终迁移到 `/data/kd_users/`（App NAS，跨发布持久）(2026-03-24)
  ...
  - [x] Job History — `/data/kd_users/` 记录历史，折叠面板展示 (2026-03-24)
  - [x] 合并合作者 `3e650b6` — sympy 渲染、Regression viz、BrowserState 兼容 (2026-03-24)
  - [x] 修复 Viz 下拉菜单 FC 环境验证失败 — `allow_custom_value=True` (2026-03-24)
  - [x] 存储路径全面排查 — `/personal/` `/share/` `/home/` 均不持久，最终确定 `/data/` App NAS (2026-03-24)
  - [x] 持久化迁移 `/personal/.kd/` → `/data/kd_users/` — 跨发布持久保存 (2026-03-24)
  - [x] 对齐官方 SDK 文档 — download API、output_dir/remote_target 统一为 `"output"` (2026-03-24)
  - [x] 修复 SDK 下载 `s/` 子目录问题 — result.json/equation.png 多路径搜索 (2026-03-24)
  - [x] History 三层恢复 — job_list.txt + API 实时查询 + /home/outputs/ 扫描兜底 (2026-03-24)
  - [x] 恢复的 job 自动补 history 条目 (2026-03-24)
  - [x] BrowserState 固定 secret — FC 重启后 localStorage 可正常解密恢复 job_id (2026-03-24)
  - [x] 下载后递归列出 `s/` 内容 — 日志一轮即可定位 SDK 文件布局 (2026-03-24)
  - [x] `find_active_job` 用 `KD_GPU_APP_KEY` 查询 — 修复 API fallback 查不到 GPU 任务 (2026-03-24)
  - [x] 修复 `launching.py` save_output 缺少 `model=model` — GPU 端 EqGPT viz 数据写入 result.json (2026-03-24)
  - [x] GPU 镜像全量文件同步 — `kd_eqgpt.py` 缺 `self.result_`、`_adapters/eqgpt.py` 完全缺失等 13+2 个文件差异 (2026-03-24)
